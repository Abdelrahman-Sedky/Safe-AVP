...
{
/* FHA (Receive error data)
    *    1) It checks if the response is valid and if there are any errors.
    *    2) If the response is invalid or if the map service request fails, it generates a runtime error.
    *    3) Otherwise, it proceeds with the planning process and checks if the generated trajectory is valid.
	
		  auto response = future.get();
		  
		  if (response == nullptr) {
			RCLCPP_ERROR(get_logger(), "Received invalid map response");
			throw std::runtime_error("Invalid map response");
		  }
		  if (response->error_code != HADMapService::Response::SUCCESS) {
			RCLCPP_ERROR(get_logger(), "Map service request failed with error code: %d", response->error_code);
			throw std::runtime_error("Map service request failed");
		  }
